...
plt.xscale('log')
plt.yscale('log')
#plt.title('raw times for use case number %d' % 3, fontsize=30)
plt.xlabel('number of cells', fontsize=30)
plt.ylabel('raw time [seconds]', fontsize=30)
plt.xlim(numberOfCells[0], numberOfCells[-1])
plt.setp(ax2d.get_xticklabels(), fontsize=20)
plt.setp(ax2d.get_yticklabels(), fontsize=20)
#plt.tight_layout()

lgd = ax2d.legend(labels, loc='center right', bbox_to_anchor=bbox_to_anchor2d)
#fig2d.savefig('samplefigure', bbox_extra_artists=(lgd,), bbox_inches='tight')
