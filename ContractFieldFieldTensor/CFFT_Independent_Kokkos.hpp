...
struct contractFieldFieldTensorFunctor {
  ...
  KOKKOS_INLINE_FUNCTION
	  void operator() (const unsigned int elementIndex) const {
		  ...
		  const unsigned int myCell = elementIndex % _numCells;
		  const unsigned int matrixIndex = elementIndex / _numCells;
		  const unsigned int lbf = matrixIndex / _numRightFields;
		  const unsigned int rbf = matrixIndex % _numRightFields;
			  ...
			  for (int iTens1 = 0; iTens1 < _dim1Tens; iTens1++) {
				  for (int iTens2 = 0; iTens2 < _dim2Tens; iTens2++) {
					  temp += _leftFields(myCell, lbf, qp, iTens1, iTens2) *
						  _rightFields(myCell, rbf, qp, iTens1, iTens2);
				  }
			  }
		  }
