...
prefix = 'data/ContractFieldFieldTensor_'
...
tensorSize = numpy.loadtxt(open(prefix + 'tensorSize' + suffix + '.csv','rb'),delimiter=',',skiprows=0)
memorySize = numpy.loadtxt(open(prefix + 'memorySize' + suffix + '.csv','rb'),delimiter=',',skiprows=0)
numberOfTensors = numpy.loadtxt(open(prefix + 'numberOfTensors' + suffix + '.csv','rb'),delimiter=',',skiprows=0)
...
surf = ax.plot_surface(log10(tensorSize), log10(memorySize), log10(numberOfTensors), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
plt.xlabel('log10(tensorSize)')
plt.ylabel('log10(memorySize)')
ax.set_zlabel('log10(numberOfTensors)')
plt.title('number of dot products')
if (makeImageFiles == True):
  ax.view_init(elev=2, azim=-23)
  filename = outputPrefix + 'NumberOfTensors' + suffix
...
for timesIndex in range(len(allTimes)):
  ...
  ax = fig3d.gca(projection='3d')
  ax.view_init(elev=0, azim=-111)
  surf = ax.plot_surface(log10(tensorSize), log10(memorySize), log10(times), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
...
for memorySizeIndex in [-1, 0]:
  ...
  for timesIndex in range(len(allTimes)):
    times = allTimes[timesIndex]
    name = allNames[timesIndex]
    plt.plot(tensorSize[:, memorySizeIndex], times[:, memorySizeIndex], markers[timesIndex], color=colors[timesIndex], hold='on', linewidth=2)
  ...
  plt.xlim([tensorSize[0, 0], tensorSize[-1, 0]])
...
for timesIndex in range(len(allTimes)):
  ...
  ax = fig3d.gca(projection='3d')
  ax.view_init(elev=0, azim=-111)
  surf = ax.plot_surface(log10(tensorSize), log10(memorySize), log10(times / memorySize), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
  surf.set_norm(colorNormalizer)
  plt.xlabel('log10(tensorSize)')
  plt.ylabel('log10(memorySize)')
...
for timesIndex in numpy.arange(1, len(allTimes)):
  ...
  ax = fig3d.gca(projection='3d')
  ax.view_init(elev=0, azim=-111)
  surf = ax.plot_surface(log10(tensorSize), log10(memorySize), log10(allTimes[0] / times), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
  surf.set_norm(colorNormalizer)
  plt.xlabel('log10(tensorSize)')
  plt.ylabel('log10(memorySize)')
...
for memorySizeIndex in [-1, 0]:
  ...
  for timesIndex in range(len(allTimes)):
    times = allTimes[timesIndex]
    name = allNames[timesIndex]
    plt.plot(tensorSize[:, memorySizeIndex], allTimes[0][:, memorySizeIndex] / times[:, memorySizeIndex], markers[timesIndex], color=colors[timesIndex], hold='on', linewidth=2)
    legendNames.append(name)
  plt.xscale('log')
  plt.yscale('log')
...
ax = fig3d.gca(projection='3d')
ax.view_init(elev=0, azim=-111)
surf = ax.plot_surface(log10(tensorSize), log10(memorySize), (allTimes[1] / allTimes[5]), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
plt.xlabel('log10(tensorSize)')
plt.ylabel('log10(memorySize)')
...
ax = fig3d.gca(projection='3d')
ax.view_init(elev=0, azim=-111)
surf = ax.plot_surface(log10(tensorSize), log10(memorySize), (allTimes[2] / allTimes[6]), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0.5, antialiased=False)
plt.xlabel('log10(tensorSize)')
plt.ylabel('log10(memorySize)')
ax.set_zlabel('speedup [unitless]')
