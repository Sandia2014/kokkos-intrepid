...
struct ContractDataDataTensorFunctor {
  ...
  // Team setup
  typedef float value_type; //Specify value type for reduction target, sum
  //typedef Kokkos::DefaultExecutionSpace::scratch_memory_space shared_space;
  //typedef Kokkos::View<float*,shared_space,Kokkos::MemoryUnmanaged> shared_1d_float;

  //shared_1d_float local_scalar = 0;
...
runKokkosTest(const unsigned int numberOfRepeats,
  ...
  //const Kokkos::TeamPolicy<> policy(numCells, Kokkos::TeamPolicy<>::team_size_max(contractDataDataTensorFunctor));
  const Kokkos::TeamPolicy<> policy(numCells, dim1 * dim2 );
  //const Kokkos::TeamPolicy<> policy(numCells, numPoints);
  ...
  for (unsigned int repeatIndex = 0;
    ...
    *totalNumberOfRepeats = *totalNumberOfRepeats + 1;
    ...
    Kokkos::parallel_for(policy, contractDataDataTensorFunctor);
